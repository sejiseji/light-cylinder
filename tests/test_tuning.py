...
def test_observation_tuning_starts_at_current_stage_one_values() -> None:
    ...
    assert tuning.fireflies == MENU_STAGE_MIN
    ...
    assert tuning.firefly_count == MENU_FIREFLY_COUNTS[0]
...
def test_observation_tuning_adjusts_and_clamps_stages() -> None:
    ...
    tuning.adjust_stage("fireflies", 9)
    assert tuning.fireflies == MENU_STAGE_MAX
    assert tuning.firefly_count == MENU_FIREFLY_COUNTS[-1]
